...
5. **Cross-cloud stitching goes through generic pivots** (`GenericIpAddress`, `GenericHostname`), built only via `Node::ip`/`Node::hostname` (see Helpers). Edge kind is load-bearing: `RoutesTo` = traffic, `ResolvesTo` = DNS, `ConnectsTo` = the resource *holds* this address. `derive::service` treats `ConnectsTo` as ownership, so DNS producers must use `ResolvesTo`, in one shape: `GenericHostname(name) -ResolvesTo-> <record> -ResolvesTo-> target`. **Every projected edge needs an owned endpoint** — `carry_forward` scopes edges by their owners, so an edge between two pivots would survive any provider's outage (`every_projected_edge_has_an_owned_endpoint`). `GraphBuilder` dedups identical pivots across clouds via its `HashMap<Node, NodeIndex>`.
